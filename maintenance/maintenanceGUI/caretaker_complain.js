...
document.getElementById('caretakerLogout').addEventListener('click', function () {
        ...
        .then(res =>
            res.json()).then(d => {
                ...
                for (let i = 0; i < d.length; i++) {
                    ...
                    if (prevCnt == cntSameDate && d[i].designation == data.ctype) {
...
fetch('http://localhost:3005/complaint/getAllComplaints', {
    ...
    .then(res =>
        res.json()).then(d => {
            ...
            document.getElementById('allCaretakerComplaints').innerHTML = str;
        })
    .catch((err) => {
        console.log(err);
    });